//Pd + Processing visuals system for CWDFP class, Autumn 2016, Aalto University
//Sourya Sen, New Media
...
//Arrays to hold glitch variables
...
char keychar = '1'; //pattern generator
...
void pattern06(){
 ...
 for(int y1=100; y1<=height; y1=y1+100){
    for(float x=0; x<=width; x=x+step){
      float y=height/2 + (srandom(-540, y1) * 0.5);
      float lasty=height/2 + (srandom(-540, y1) * 0.5);
...
//THE ACTUAL CIRCLES
void rEllipse(float cr, float cg, float cb){
  randomx();
  randomy();
  noStroke();
  fill(cr, cg, cb, 255);
